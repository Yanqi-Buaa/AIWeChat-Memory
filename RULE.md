...
**强制规则**：当用户请求中包含「方案、计划、制定、设计、规划、路线图」等关键词时，**必须先调用 info-gate 技能**进行信息缺口扫描，禁止跳过扫描直接生成方案。

- 触发词：方案、计划、plan、scheme、制定、设计、规划、proposal、路线图、roadmap
- 流程：扫描缺口 → ≤5 逐题追问 / >5 输出问卷 → 确认闭合 → 生成
- 禁止：用假设填补缺口、缺口未闭合就输出、用户说「随便」时反复追问（给建议后继续）
- 例外：简单事实查询、一次性回答、用户明确说「快速草稿」

详细规则见技能文件：`skills/info-gate/SKILL.md`
