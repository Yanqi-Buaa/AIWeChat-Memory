# -*- coding: utf-8 -*-
"""
Clean rebuild: Restore table, then build full document with python-docx only.
EMF images are converted to PNG first.
"""
import os, sys
...
# ====== CONVERT EMF TO PNG using PowerPoint ======
print("Converting EMF to PNG...")
import win32com.client as win32
from win32com.client import constants as w32const
...
for f in sorted(os.listdir(img_dir)):
    if f.lower().endswith('.emf'):
        ...
        png_path = os.path.join(img_dir, f.replace('.emf', '_conv.png'))
        print(f"  {f} -> {os.path.basename(png_path)}")
        try:
            pres = pp.Presentations.Add()
            slide = pres.Slides.Add(1, 1)  # ppLayoutBlank
            pic = slide.Shapes.AddPicture(emf_path, False, True, 0, 0)
            pic.Export(png_path, w32const.ppShapeFormatPNG)
            pres.Close()
            emf_png_map[f] = png_path
        except Exception as e:
            print(f"  ERROR: {e}")
pp.Quit()

# Also add the original PNG (image1.png is already PNG)
emf_png_map['image1.png'] = os.path.join(img_dir, 'image1.png')

print("Conversion done.")

# ====== FIX TABLE (restore to original) ======
table = doc.tables[0]
# Row 1: 应用单位 | XXXX
for cell in table.rows[1].cells:
    for p in cell.paragraphs:
        p.clear()
table.rows[1].cells[0].paragraphs[0].add_run('应用单位')
table.rows[1].cells[1].paragraphs[0].add_run('XXXX')

# Row 2: 通讯地址 | XXXX
for cell in table.rows[2].cells:
    for p in cell.paragraphs:
        p.clear()
table.rows[2].cells[0].paragraphs[0].add_run('通讯地址')
table.rows[2].cells[1].paragraphs[0].add_run('XXXX')

# Row 3: 应用起止时间 | XX年XX月至2025年8月
for cell in table.rows[3].cells:
    for p in cell.paragraphs:
        p.clear()
table.rows[3].cells[0].paragraphs[0].add_run('应用起止时间')
table.rows[3].cells[1].paragraphs[0].add_run('2023年9月至2025年8月')

# Row 4: 负责人签名...
for cell in table.rows[4].cells:
    for p in cell.paragraphs:
        p.clear()
table.rows[4].cells[0].paragraphs[0].add_run('负责人   签名                      应用单位   公章       \n    \n         年    月    日                     年    月    日')
table.rows[4].cells[1].paragraphs[0].add_run('负责人   签名                      应用单位   公章       \n    \n         年    月    日                     年    月    日')

print("Table restored.")

# ====== HELPER: Insert elements after paragraph ======
...
def make_image_para(img_path, width_inches=5.5):
    """Create a w:p with centered image."""
    # Read image
    with open(img_path, 'rb') as f:
        blob = f.read()
    
    image_part = doc.part.get_or_add_image(img_path)
    ...
    drawing_xml = (
        ...
        f'<pic:nvPicPr><pic:cNvPr id="0" name="Picture"/><pic:cNvPicPr/></pic:nvPicPr>'
    ...
    p.append(pPr)
    r = OxmlElement('w:r')
    r.append(parse_xml(drawing_xml))
    p.append(r)
    return p

def insert_all(ref_para, items):
    """Insert a list of elements after ref_para.
    items: list of (type, arg1, arg2) where type is 'text' or 'img'
    """
    current = ref_para._element
    for item_type, arg1, arg2 in items:
        if item_type == 'text':
            elem = make_para(arg1, arg2)  # style, text
        elif item_type == 'img':
            elem = make_image_para(arg1, arg2)  # path, width
...
cg1 = ('该项成果主要针对某型小涵道比涡扇发动机的研制需求，重点围绕其机动飞行环境及复杂的进排气条件，'
...
cg2 = ('该项成果主要应用于地面台架、高空台及外场飞行试验全过程的监视与诊断。'
...
cg3 = ('该项成果采用短时傅里叶变换实现高频采样信号的频域转化与数据压缩，'
...
# Determine PNG paths for figures
fig_pngs = {
    1: emf_png_map.get('image2.emf', os.path.join(img_dir, 'image2_conv.png')),
    2: emf_png_map.get('image1.png', os.path.join(img_dir, 'image1.png')),
    3: emf_png_map.get('image3.emf', os.path.join(img_dir, 'image3_conv.png')),
    4: emf_png_map.get('image4.emf', os.path.join(img_dir, 'image4_conv.png')),
    5: emf_png_map.get('image5.emf', os.path.join(img_dir, 'image5_conv.png')),
    6: emf_png_map.get('image6.emf', os.path.join(img_dir, 'image6_conv.png')),
}

fig_captions = {
    1: '图1 横向惯性载荷作用下整机承力结构变形引起支点不同心',
    2: '图2 整机结构系统弹性线示意图',
    3: '图3 某高推重比涡扇发动机在试飞中出现的振动',
    4: '图4 某高推重比涡扇发动机外场飞行振动特征提取与分析',
    5: '图5 本项目研究成果在某高推重比涡扇发动机上的应用效果',
    6: '图6 典型振动特征参数数据库示意图',
}
...
# Insert: [图1_cap] [图1_img] [图2_cap] [图2_img] [cg1_end]
insert_all(p32, [
    ('text', '0-图题', fig_captions[1]),
    ('img', fig_pngs[1], 5.5),
    ('text', '0-图题', fig_captions[2]),
    ('img', fig_pngs[2], 5.5),
    ('text', '0-报告正文', cg1_end),
])
...
insert_all(p34, [
    ('text', '0-图题', fig_captions[3]),
    ('img', fig_pngs[3], 5.5),
    ('text', '0-图题', fig_captions[4]),
    ('img', fig_pngs[4], 5.5),
    ('text', '0-图题', fig_captions[5]),
    ('img', fig_pngs[5], 5.5),
    ('text', '0-报告正文', cg2_end),
])
...
insert_all(p36, [
    ('text', '0-图题', fig_captions[6]),
    ('img', fig_pngs[6], 5.5),
])

# 成果应用结论 [37]
p37 = doc.paragraphs[37]
p37._element.addnext(make_para('0-报告正文', conclusion))
