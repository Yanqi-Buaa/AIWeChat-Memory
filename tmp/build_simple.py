# -*- coding: utf-8 -*-
"""
Strategy: python-docx for text content, Word COM for cut/paste of field codes and images.
Steps:
1. python-docx: insert body text as runs in correct paragraphs
2. Word COM: find end-section content, cut and paste to correct positions
"""
...
# ====== STEP 1: Write body text using python-docx ======
print("Step 1: Writing body text...")
doc = Document(target)

def make_para_element(style, text):
...
def add_elements_after(para, elements):
    """Insert list of elements after para, return last element."""
...
# Add three成果 as separate paragraphs
add_elements_after(p30, [
    make_para_element('0-报告正文', 
...
# After the text, place marker for figure insertion
p_cg1_end = add_elements_after(p32, [
    make_para_element('0-报告正文', '【图1+图2插入位置】'),
    make_para_element('0-报告正文',
...
p_cg2_end = add_elements_after(p34, [
    make_para_element('0-报告正文', '【图3+图4+图5插入位置】'),
    make_para_element('0-报告正文',
...
p36.add_run(
    ...
    '如【图6】所示。随着数据持续积累，为服役后的异常边界判定与精细化健康管理奠定了基础。'
)

p_cg3_end = add_elements_after(p36, [
    make_para_element('0-报告正文', '【图6插入位置】'),
    make_para_element('0-报告正文',
...
p37._element.addnext(make_para_element('0-报告正文',
...
doc.save(tmp_path)
print("Step 1 complete. Saved intermediate.")

# ====== STEP 2: Word COM - Cut end content and paste at markers ======
print("\nStep 2: Moving cross-refs and figure captions via Word COM...")
...
try:
    ...
    # Move figure captions from end to position markers
    fig_positions = [
        ('图1 横向惯性载荷', '【图1+图2插入位置】'),
        ('图2 整机结构系统', '【图1+图2插入位置】'),
        ('图3 某高推重比涡扇', '【图3+图4+图5插入位置】'),
        ('图4 某高推重比涡扇', '【图3+图4+图5插入位置】'),
        ('图5 本项目研究成果', '【图3+图4+图5插入位置】'),
        ('图6 典型振动特征', '【图6插入位置】'),
    ]
    
    for cap_find, marker in fig_positions:
        # Find caption at end, select it
        find_select(cap_find)
        sel.MoveUp(Unit=wdLine, Count=1, Extend=1)
        sel.Copy()
        
        # Find marker and paste before it
        find_select(marker)
        sel.Paste()
        sel.TypeParagraph()
    
    # Now move cross-references
    xref_map = [
        ('【图1】', '图1'),
        ('【图2】', '图2'),
        ('【图3】', '图3'),
    ...
    for marker_char, ref_text in xref_map:
        find_select(ref_text)  # Find the REF field text at end
        sel.MoveUp(Unit=wdLine, Count=1, Extend=1)
        sel.Copy()
        
        find_select(marker_char)  # Find marker in body
        sel.Paste()
    
    # Delete all marker placeholders
    for marker in ['【图1+图2插入位置】', '【图3+图4+图5插入位置】', '【图6插入位置】',
                   '【图1】', '【图2】', '【图3】', '【图4】', '【图5】', '【图6】']:
        find_select(marker)
        sel.TypeText('')  # Delete it
    
    # Delete the end section content (cross-ref list and figure captions)
    # and the "交叉引用：" text
    find_select('交叉引用：')
    sel.MoveDown(Unit=wdLine, Extend=1)
    # Extend selection to cover until the very end
    for _ in range(30):
        sel.MoveDown(Unit=wdLine, Extend=1)
    sel.TypeText('')  # Delete all selected
