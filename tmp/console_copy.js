...
const I18N = {
    zh: {
        ...
        config_thinking_effort: '思考强度', config_thinking_effort_hint: '思考模式的深度级别，越高思考越深入但耗时越长',
        thinking_effort_low: '低', thinking_effort_medium: '中', thinking_effort_high: '高 (推荐)', thinking_effort_max: '最强',
    ...
    en: {
        ...
        config_thinking_effort: 'Thinking Effort', config_thinking_effort_hint: 'Depth of thinking; higher means deeper reasoning but longer response time',
        thinking_effort_low: 'Low', thinking_effort_medium: 'Medium', thinking_effort_high: 'High (Recommended)', thinking_effort_max: 'Max',
...
function _addOptimisticSessionItem(sid) {
    ...
    item.innerHTML = `
        ...
        <span class="session-channel-badge">网页</span>
        <span class="session-title" title="${escapeHtml(title)}">${escapeHtml(title)}</span>
        <button class="session-rename" onclick="event.stopPropagation(); startRenameSession('${sid}')" title="${currentLang === 'zh' ? '重命名' : 'Rename'}">
...
function _channelLabel(ch) {
    const map = { weixin: '微信', web: '网页', terminal: '终端', feishu: '飞书', dingtalk: '钉钉', wecom_bot: '企微', qq: 'QQ', wechatcom_app: '企微', wechatmp: '公众号', wechatmp_service: '公众号' };
    return map[ch] || ch;
}
    ...
    fetch(`/api/sessions?page=${page}&page_size=${_SESSION_PAGE_SIZE}`)
        ...
        .then(data => {
            ...
            sessions.forEach(s => {
                ...
                const chLabel = _channelLabel(s.channel_type || '');
                const badgeHtml = chLabel ? `<span class="session-channel-badge">${escapeHtml(chLabel)}</span>` : '';
                item.innerHTML = `
                    <i class="fas fa-message session-icon"></i>
                    ${badgeHtml}
                    <span class="session-title" title="${escapeHtml(title)}">${escapeHtml(title)}</span>
                    <button class="session-rename" onclick="event.stopPropagation(); startRenameSession('${s.session_id}')" title="${currentLang === 'zh' ? '重命名' : 'Rename'}">
                        <i class="fas fa-pen-to-square"></i>
                    </button>
...
function initConfigView(data) {
    ...
    const thinkingEffort = data.thinking_effort || 'high';
    initThinkingEffortDropdown(thinkingEffort);
    onThinkingToggle();
...
function saveAgentConfig() {
    const updates = {
        ...
        thinking_effort: getThinkingEffortValue(),
    };
...
function onThinkingToggle() {
    const wrap = document.getElementById('cfg-thinking-effort-wrap');
    const checked = document.getElementById('cfg-enable-thinking').checked;
    wrap.classList.toggle('hidden', !checked);
}

function initThinkingEffortDropdown(initialValue) {
    const el = document.getElementById('cfg-thinking-effort-select');
    if (!el) return;
    const options = [
        { value: 'low', label: t('thinking_effort_low') },
        { value: 'medium', label: t('thinking_effort_medium') },
        { value: 'high', label: t('thinking_effort_high') },
        { value: 'max', label: t('thinking_effort_max') },
    ];
    initDropdown(el, options, initialValue || 'high', null);
}

function getThinkingEffortValue() {
    const el = document.getElementById('cfg-thinking-effort-select');
    return (el && el._ddValue) ? el._ddValue : 'high';
}
