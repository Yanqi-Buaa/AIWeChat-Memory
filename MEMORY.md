...
## 待开发功能
- **Agent输出路由选择**（2026-05-12提出，YQ认为两个需求可收敛）：给Agent一个可选接口，指定消息发送目标——默认当前对话，可选外部渠道（微信/群聊/其他session）。这是以下两个功能的底层共同能力：
  - **跨会话任务监控**：监控其他session的任务进度，完成后回调通知。核心难点：session状态感知 + 回调路由
  - **定时任务发送端指定**：创建定时任务时可指定发送渠道/接收端（如"明天8点往XX群发消息"），而非绑死当前会话
