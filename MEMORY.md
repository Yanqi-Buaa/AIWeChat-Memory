...
## 已实现功能
- **Agent输出路由选择**（2026-05-13验证通过）：scheduler 任务通过 `route` 参数可指定发送目标（微信/群聊/其他session），微信 user_id 从 `~/.weixin_cow_credentials.json` 获取。详见 `A:\AIWeChat\CowAgent\FEATURES.md`

## 待开发功能
- **跨会话任务监控**：监控其他session的任务进度，完成后回调通知。核心难点：session状态感知 + 回调路由
- **看门狗托管重启**：Agent离线自更新后由外部看门狗（Windows任务计划程序）托管重启，规避cow restart自杀失败导致静默死亡。架构：pending_restart标记 + pending_notify.json启动钩子
